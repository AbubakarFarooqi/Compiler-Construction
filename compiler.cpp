...
using ASTNodePtr = shared_ptr<ASTNode>; // Define ASTNodePtr before ASTNode, shared_ptr is smrt pointer that auto destroy
...
public:
    ...
    void declareVariablesInDataSegment()
    {
        ...
        {
            if (sym.second.dataType == "int" || sym.second.dataType == "mera_poo")
            {
...
int main(int argc, char **argv)
{
    ...
    // output file;
    ofstream outFile;
    outFile.open((string(argv[2]) + ".asm"));
    ...
    TACGenerator tacGen;
    auto tac = tacGen.generateTAC(node);
    ...
    outFile << asembly.getAssembly();
    outFile.close();

    // Assemble the source file with an explicit include path
    int assembleResult = system(("ml /I C:\\Irvine /c /Fl /Fo" + string(argv[2]) + ".obj " + string(argv[2]) + ".asm").c_str());
    if (assembleResult != 0)
    {
        std::cerr << "Error during assembly. Check your source file or MASM setup." << std::endl;
        return 1;
    }

    // Link the object file
    int linkResult = system(("link /SUBSYSTEM:CONSOLE /MACHINE:X86 /OUT:" + string(argv[2]) + ".exe " + string(argv[2]) + ".obj Irvine32.lib /LIBPATH:C:\\Irvine kernel32.lib user32.lib").c_str());
    if (linkResult != 0)
    {
        std::cerr << "Error during linking. Check library paths and dependencies." << std::endl;
        return 1;
    }

    std::cout << "Build successful. Executable created as output.exe" << std::endl;
